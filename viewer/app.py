...
def token_error_response(callback):
    ...
    app_logger.warning(f"JWT error: {callback}. Redirecting to login service.")
...
def index():
    ...
    app_logger.info(f"User {current_user_identity} accessed FormViewerService.")
...
def logout():
    ...
    app_logger.info("User logged out and JWT cookies unset.")
    return response
...
def get_forms_api():
    ...
    app_logger.info("API request for all forms received.")
...
def get_form_by_id_api(form_id):
    ...
    if form:
        app_logger.info(f"API request for form ID: {form_id} received.")
        return jsonify(form)
    app_logger.warning(f"Form with ID: {form_id} not found.")
...
if __name__ == '__main__':
    ...
    os.environ.setdefault('FLASK_SECRET_KEY', 'local-dev-secret-form-viewer')
    os.environ.setdefault('JWT_SECRET_KEY', 'local-dev-jwt') # Ensure this matches your login service's local dev key
    os.environ.setdefault('JWT_COOKIE_DOMAIN', 'localhost')
    os.environ.setdefault('LOGIN_SERVICE_URL', 'http://localhost:8080/login')
    ...
    try:
        app.run(
            ...
            port=port,
            threaded=True,
            use_reloader=False # Important: disable reloader in production for stability
        )
    except Exception as e:
        app_logger.error(f"Error starting Flask app: {e}", exc_info=True) # exc_info=True prints traceback
        raise
