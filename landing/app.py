...
def user_info():
    try:
        claims = get_jwt()
        user_email = claims.get('sub')
        user_name = claims.get('user_name', user_email)
        if user_email:
...
def health_check():
    ...
    login_service_status = 'not_configured'
    if login_service_client and app.config.get('LOGIN_SERVICE_URL'):
        try:
            response = login_service_client.make_authenticated_request('/health')
            login_service_status = 'ready' if response.status_code == 200 else f'error: {response.status_code}'
        except Exception as e:
            login_service_status = f'error: {str(e)}'
            app_logger.error(f"Login service health check failed: {str(e)}")
    health_status['checks']['login_service_reachability'] = login_service_status
